...
const NotificationContactDetails = ({navigation, route}) => {
  ...
  const sendNotesToApi = async () => {
    ...
    try {
      ...
      console.log('form datata', data);

      const response = await _post(`/leads/notes/save/${item}`, data);
      console.log('tesxe', response);
    } catch (error) {
    ...
    if (Platform.OS != 'ios') {
      try {
        ...
        if (granted === PermissionsAndroid.RESULTS.GRANTED) {
          ...
          // CallLogs.load(3).then(c => console.log('callllllllll', c));
        } else {
  ...
  const getAllNotes = async () => {
    try {
      const response = await _get(`/getnotes/${item}`);
  ...
  const getLeadView = async () => {
    try {
      const response = await _get(`/leadview/${item}`).then(response => {
...
export default NotificationContactDetails;
